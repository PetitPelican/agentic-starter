...
2. Lancer le script en dry-run depuis la racine du projet à migrer :
   ```powershell
   .\.codex\skills\project-upgrade\scripts\project-upgrade.ps1
   ```

   Si le projet n'a pas encore `.codex/`, lancer le script depuis un clone local de `agentic-starter` :
   ```powershell
   <CHEMIN_AGENTIC_STARTER>\.codex\skills\project-upgrade\scripts\project-upgrade.ps1 -ProjectRoot <CHEMIN_PROJET>
   ```
...
4. Appliquer uniquement après accord :
   ```powershell
   .\.codex\skills\project-upgrade\scripts\project-upgrade.ps1 -Apply
   ```
